...
export class RelationalUtil {
  ...
  /**
 * Calculate related items by count of shared relationships across collections.
 * @param {string} collectionName - The collection of the target item.
 * @param {string} slug - The slug of the target item.
 * @returns {array} - Sorted array of related items with counts.
 */
getRelatedItemsByCount(collectionName, slug) {
  const targetEntity = this.findEntityBySlug(collectionName, slug);
  if (!targetEntity) return [];

  const relationshipCounts = {};

  // Iterate through collections to calculate shared relationships
  this.content.collections.forEach((collection) => {
    if (collection.collection === collectionName) return; // Skip the same collection

    collection.items.forEach((item) => {
      // Check for shared relationships
      Object.keys(item).forEach((key) => {
        if (key.startsWith("relatedTo")) {
          const relatedSlugs = item[key];
          if (relatedSlugs.includes(slug)) {
            relationshipCounts[item.slug] = (relationshipCounts[item.slug] || 0) + 1;
          }
        }
      });
    });
  ...
  // Convert to array, sort by count, and return
  return Object.entries(relationshipCounts)
    .map(([slug, count]) => {
      const relatedItem = this.findEntityBySlug(collectionName, slug);
